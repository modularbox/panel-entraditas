...
describe("OrderDetailPage", () => {
  ...
  it("shows the order header, customer, and its line items with the total", async () => {
    ...
    expect(screen.getAllByText("220,00 €").length).toBeGreaterThan(0); // subtotal y total
  });
  ...
  it("hides the refund form for a user without orders:refund", async () => {
    await useSessionStore.getState().login("marta.gutierrez@entraditas.com", "marta1234"); // role "user", scoped to event-1/event-2
