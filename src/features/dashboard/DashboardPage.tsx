...
function LineChart({ points }: { points: DashboardOverview["salesTimeline"] }) {
  ...
  return <div><svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-44 w-full overflow-visible" role="img" aria-label="Ventas acumuladas y proyecciÃ³n"><line x1="0" y1="100" x2="100" y2="100" stroke="currentColor" strokeOpacity=".2" /><polyline points={projection} fill="none" stroke="#f2c14e" strokeDasharray="2 2" strokeWidth="1.5" /><polyline points={coordinates} fill="none" stroke="#e4572e" strokeWidth="2" /></svg><div className="mt-2 flex justify-between text-[11px] text-muted-foreground">{points.map((point) => <span key={point.label}>{point.label}</span>)}</div><div className="mt-3 flex gap-4 text-xs"><span><i className="mr-1 inline-block h-2 w-2 bg-primary" />Ventas</span><span><i className="mr-1 inline-block h-2 w-2 bg-accent" />ProyecciÃ³n</span></div></div>;
}
...
export function DashboardPage() {
  ...
  return <div className="flex flex-col gap-6">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Resumen operativo</p><h1 className="mt-1 font-display text-3xl font-semibold">Dashboard</h1><p className="mt-1 text-sm text-muted-foreground">Actualizado a las {refreshed} Â· sincronización automÃ¡tica cada 15 s</p></div><div className="flex items-center gap-2"><label htmlFor="report-format" className="sr-only">Formato de informe</label><select id="report-format" className="h-10 rounded-md border-2 border-foreground bg-surface px-3 text-sm"><option value="csv">CSV</option><option value="xlsx">XLSX</option><option value="pdf">PDF</option></select><Button onClick={() => exportReport((document.getElementById("report-format") as HTMLSelectElement).value)}>Exportar informe</Button></div></header>
