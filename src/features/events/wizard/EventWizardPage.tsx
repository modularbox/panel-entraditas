...
import { EventSetupQuestionnaire } from "./steps/EventSetupQuestionnaire";
...
export function EventWizardPage() {
  ...
  const isCreating = Boolean(params.id && params.id === "nuevo");
  ...
  useEffect(() => {
    ...
    if (params.id && params.id !== "nuevo") {
      ...
      reset();
      // A fresh draft replays the questionnaire, so the previous answers must not leak in.
  ...
  const steps = ALL_STEPS.filter((step) => {
    // The questionnaire answers shape the wizard before the event exists; once it is
    // loaded, the event's own fields (which Step 1 seeded from the answers) take over.
    if (step.key === "subeventos") {
      const hasSubEvents = event ? event.hasSubEvents : setup.hasSubEvents;
      if (!hasSubEvents) return false;
    }
    if (isCreating && step.key === "plano" && !setup.needsSeatingPlan) return false;
    return true;
  });
  ...
  // Crear un evento nuevo arranca con el cuestionario previo: decidir cómo se
  // configura el evento (sesiones, plano, límites) antes de entrar al asistente.
  const shouldAskSetup = isCreating && !setup.completed;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {shouldAskSetup ? (
        <EventSetupQuestionnaire onComplete={handleSetupComplete} />
      ) : (
        <>
