...
export const discountCodesHandlers = [
  ...
  http.post(`${BASE}/events/:eventId/discount-codes`, async ({ request, params }) => {
    ...
    const value = body.type === "fixed" ? Math.round(Number(body.value ?? 0) * 100) : Math.round(Number(body.value ?? 0));
  ...
  http.delete(`${BASE}/discount-codes/:id`, ({ request, params }) => {
    ...
    if (result.discount.usedCount > 0) {
      result.discount.status = "inactive";
      return HttpResponse.json({ data: result.discount, meta: { requestId: "req_discounts_inactive" } });
    }
