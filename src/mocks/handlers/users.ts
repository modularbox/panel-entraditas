...
function validateAssignment(actor: User, effective: Set<string>, role: RoleSlug, overrides: PermissionOverride[], scopes: string[]) {
  // An organization has exactly one admin (its primary account, provisioned by the platform);
  // team management only ever creates user and subuser.
  if (role === "admin") return "Una organización solo puede tener un administrador";
