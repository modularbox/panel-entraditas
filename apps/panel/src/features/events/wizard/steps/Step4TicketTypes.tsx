...
export function Step4TicketTypes({ eventId, goNext }: Step4TicketTypesProps) {
  ...
  const hasUnsavedTicketDraft =
    ...
    color !== TICKET_COLOR_PALETTE[groups.length % TICKET_COLOR_PALETTE.length] ||
    scopeMode !== "event" ||
  ...
  function handleContinue() {
    setError(null);
    if (editingGroupId) {
      setError("Guarda o cierra la edicion del tipo de entrada antes de continuar.");
      return;
    }
    if (hasUnsavedTicketDraft) {
      setError("Tienes un tipo de entrada sin guardar. Guardalo o vacia el formulario antes de continuar.");
      return;
    }
    goNext();
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Button type="button" onClick={handleContinue} className="mt-4 self-start">
        Continuar
      </Button>
    </div>
  );
