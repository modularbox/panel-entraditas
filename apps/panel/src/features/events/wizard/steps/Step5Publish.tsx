...
function readPlan(eventId: string | null): VenuePlanElement[] {
  if (typeof window === "undefined") return [];
  try {
    const raw = window.localStorage.getItem(eventPlanKey(eventId));
    return raw ? (JSON.parse(raw) as VenuePlanElement[]) : [];
  } catch {
    return [];
  }
}
...
export function Step5Publish({ eventId }: Step5PublishProps) {
  ...
  const [planElements, setPlanElements] = useState<VenuePlanElement[]>(() => readPlan(eventId));

  useEffect(() => {
    setPlanElements(readPlan(eventId));
  }, [eventId]);
