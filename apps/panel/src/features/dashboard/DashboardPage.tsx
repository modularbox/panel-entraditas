...
export function DashboardPage() {
  ...
  async function exportReport(format: string) {
    ...
    try { await apiClient.post("/reports/export", { report: "dashboard", format }, { token: token! }); setExportMessage(`Exportación ${format.toUpperCase()} en cola.`); } catch (cause) { if (cause instanceof AppError) setExportError(cause.message); }
  }
